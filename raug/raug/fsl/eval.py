...
def metrics_for_eval_episodic(model, data_loader, device, loss_fn, topk=2, get_balanced_acc=False, get_auc=False):
    """
...
def get_predictions(model, data, device):
    def _process_predictions(model, images_batch, images_batch_sup, labels_batch_sup, meta_data_batch=None,
                         meta_data_batch_sup=None):
        with torch.no_grad():
            model.backbone.set_use_fc(False)

            if meta_data_batch is None:
                model.process_support_set(images_batch_sup, labels_batch_sup)
                pred_batch = model(images_batch)
            else:
                # TODO: That won't work. Method process_support_set (and compute_prototypes) has issues with multi input.
                model.process_support_set([images_batch_sup, meta_data_batch_sup], labels_batch_sup)
                pred_batch = model([images_batch, meta_data_batch])

            model.backbone.set_use_fc(True)
    ...
    try:
        images_batch, meta_data_batch = query_images
        labels_batch = query_labels

        images_batch_sup, meta_data_batch_sup = support_images
        labels_batch_sup = support_labels

    except ValueError:
        # images_batch, labels_batch = data
        images_batch = query_images
        labels_batch = query_labels

        images_batch_sup = support_images
        labels_batch_sup = support_labels
    ...
    labels_batch_sup = labels_batch_sup.to(device)

    if len(meta_data_batch):
        # Moving the data to the device that we set above
        images_batch = images_batch.to(device)
        images_batch_sup = images_batch_sup.to(device)
        if len(labels_batch):
            labels_batch = labels_batch.to(device)
        meta_data_batch = meta_data_batch.to(device)
        meta_data_batch = meta_data_batch.float()

        meta_data_batch_sup = meta_data_batch_sup.to(device)
        meta_data_batch_sup = meta_data_batch_sup.float()

        # Doing the forward pass using meta-data
        pred_batch = _process_predictions(model, images_batch, images_batch_sup, labels_batch_sup,
                                      meta_data_batch, meta_data_batch_sup)
    ...
    return pred_batch, labels_batch, img_id
...
def metrics_for_eval(model, data_loader, device, loss_fn, topk=2, get_balanced_acc=False, get_auc=False):
    ...
    with tqdm(total=len(data_loader), ascii=True, ncols=100) as t:
        ...
        with torch.no_grad():
            ...
            for data in data_loader:
                pred_batch, labels_batch, img_id = get_predictions(model, data, device)
