...
def cnfg():
    # Task
    _task = "TaskII"
    _task_base_path = "/dataset/data/ndb-ufes"
    _task_dict = {
      "TaskII": {
        "label": "TaskII",
        "features": [
            'localization_Tongue', 'localization_Lip', 'localization_Floor of mouth', 'localization_Buccal mucosa', 'localization_Palate', 'localization_Gingiva', 
            'larger_size',
            'tobacco_use_Yes', 'tobacco_use_Former', 'tobacco_use_No', 'tobacco_use_Not informed', 
            'alcohol_consumption_No', 'alcohol_consumption_Former', 'alcohol_consumption_Yes', 'alcohol_consumption_Not informed', 
            'sun_exposure_No', 'sun_exposure_Yes', 'sun_exposure_Not informed', 
            'gender_M', 'gender_F',
            'age_group_2', 'age_group_1', 'age_group_0'
          ],
        "path": F"{_task_base_path}",
        "img_path": F"{_task_base_path}/images",
        "img_col": "path",
      },
      "TaskIII": {
        "label": "TaskIII",
    ...
    _csv_path_train = os.path.join(_base_path, F"ndbufes_{_task}_parsed_folders.csv")
    _csv_path_test = os.path.join(_base_path, F"ndbufes_{_task}_parsed_test.csv")
    _imgs_folder_train = os.path.join(_base_path, "imagem-exemplo")


    _initial_model = "/home/leandro/Documentos/doutorado/tmp/results/sab_displasia_batch15/None_mobilenetv2_120d_reducer_90_fold_1_16606785628502579/best-checkpoint/best-checkpoint.pth"
...
def main(_folder, _csv_path_train, _imgs_folder_train, _lr_init, _sched_factor, _sched_min_lr, _sched_patience,
    ...
    # sample_shape = [sample[0].shape, sample[2].shape]
    # print(summary(model, sample_shape))
    ...
    fit_model(model, train_data_loader, val_data_loader, optimizer=optimizer, loss_fn=loss_fn, epochs=_epochs,
              epochs_early_stop=_early_stop, save_folder=_save_folder,
              initial_model=initial_model, load_backbone_only=True,
    ...
    test_model(model, test_data_loader, checkpoint_path=_checkpoint_best, loss_fn=loss_fn, metrics_to_comp="all",
               class_names=_labels_name, metrics_options=_metric_options, save_pred=True, verbose=False)
    ####################################################################################################################
