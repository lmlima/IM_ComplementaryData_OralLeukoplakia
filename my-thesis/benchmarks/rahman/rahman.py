...
import torch
from aug_rahman import ImgTrainTransform, ImgEvalTransform
import time
...
def cnfg():
    # Dataset variables
    _folder = 5  # 5
    _base_path = "/home/leandro/Documentos/doutorado/dados/histo-rahman-20/rahman20-ds"
    _csv_path_train = os.path.join(_base_path, "rahman-20_parsed_folders.csv")
    _imgs_folder_train = os.path.join(_base_path, "all_imgs")

    _csv_path_test = os.path.join(_base_path, "rahman-20_parsed_test.csv")
    ...
    _batch_size = 30  # 24; não definido na tese
    _epochs = 150
...
def main(_folder, _csv_path_train, _imgs_folder_train, _lr_init, _sched_factor, _sched_min_lr, _sched_patience,
    ...
    meta_data_columns = meta_data_columns_12features
    _label_name = "target"
    _img_path_col = "Image_id"
    ...
    # loss_fn = nn.BCELoss(weight=torch.Tensor(_weights).to(device))
