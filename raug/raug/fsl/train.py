...
def fit_model (model, train_data_loader, val_data_loader, optimizer=None, loss_fn=None, epochs=10,
               ...
               val_metrics=('balanced_accuracy', 'auc'), metric_early_stop=None, load_backbone_only=False):
    """
    This is the main function to carry out the training phase.

    :param load_backbone_only (bool): Load only backbone weights (those started with 'features.') from initial_model
    :param c_epoch (int): the current epoch
    :param t_epoch (int): the total number of epochs
    :param device (torch.device): the device to carry out the training
    :param model (torch.nn.Module): a module to be trained
    ...
    if initial_model is not None:
        ...
        if resume_train:
            model, optimizer, loss_fn, epoch_resume = load_model(initial_model, model, load_backbone_only=load_backbone_only)
            fsl_model = model
            model = fsl_model.backbone
            logger.info("Resuming the training from epoch {} ...".format(epoch_resume))
        else:
            model = load_model(initial_model, model, load_backbone_only=load_backbone_only)
            fsl_model = model
            model = fsl_model.backbone
    ...
    if device is None:
        if torch.cuda.is_available():
            ...
            if m_gpu > 1:
                ...
                fsl_model = nn.DataParallel(fsl_model)
            else:
    ...
    while epoch < epochs:
        ...
        val_metrics = metrics_for_eval (fsl_model, val_data_loader, device, loss_fn, topk,
        ...
        if save_folder is not None:
            save_model(fsl_model, save_folder, epoch, optimizer, loss_fn, best_flag, multi_gpu=m_gpu > 1)
        best_flag = False
