...
def label_categorical_to_number (data_csv, col_target, col_target_number=None, save_path=None, rename_reorder=None):
    """
    ...
    :param rename_reorder: A dict with original label in keys and new label in values. Order of the items in dict will
    define order of label number.
    :param data_csv (string or pd.DataFrame): the path for a csv or a dataframe already loaded
    :param col_target (string): the name of the target/label column
    ...
    if rename_reorder is not None:
        data_csv[col_target] = data_csv[col_target].astype('category').cat.rename_categories(rename_reorder).cat.reorder_categories(rename_reorder.values())
