...
class MyDatasetSSL(data.Dataset):
    """
...
def get_data_loader_ssl(imgs_path, labels, meta_data=None, transform=None, batch_transform=None, batch_size=30, shuf=True,
                    num_workers=4,
                    pin_memory=True, drop_last=False, returnDataset=False):
    """
    ...
    dt = MyDatasetSSL(imgs_path, labels, meta_data, transform)
    dl = data.DataLoader(dataset=dt, batch_size=batch_size, shuffle=shuf, num_workers=num_workers,
                         pin_memory=pin_memory, drop_last=drop_last, collate_fn=get_collate(batch_transform))
    if returnDataset:
        return dl, dt
    else:
        return dl
...
def get_data_loader(imgs_path, labels, meta_data=None, transform=None, batch_transform=None, batch_size=30, shuf=True,
                    num_workers=4,
                    pin_memory=True, drop_last=False, returnDataset=False):
    """
    This function gets a list og images path, their labels and meta-data (if applicable) and returns a DataLoader
